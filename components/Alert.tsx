...
const Alert: React.FC<AlertProps> = ({ message, type, onClose }) => {
  const baseClasses = 'p-4 rounded-md shadow-lg flex items-start';
  const typeClasses = {
  ...
  return (
    <div className={`${baseClasses} ${typeClasses[type]}`} role="alert">
      <Icon type={type} />
      <div className="flex-grow min-w-0 break-words">{message}</div>
      {onClose && (
        <button
          onClick={onClose}
          className="ml-4 p-1 rounded-md hover:bg-opacity-20 hover:bg-white focus:outline-none focus:ring-2 focus:ring-white flex-shrink-0"
